...
namespace DanfeSharp
{
    ...
    public static class Formatador
    {
        ...
        public static string FormatarCEP(string cep) => InternalRegexReplace(cep, CEP, "$1-$2");
        ...
        public static string FormatarCnpj(string cnpj) => InternalRegexReplace(cnpj, CNPJ, "$1.$2.$3/$4-$5");

        public static string FormatarCpf(string cpf) => InternalRegexReplace(cpf, CPF, "$1.$2.$3-$4");
        ...
        public static string Formatar(this int number, string formato = FormatoMoeda) => number.ToString(formato, Cultura);

        public static string Formatar(this int? number, string formato = FormatoMoeda) => number.HasValue ? number.Value.Formatar(formato) : string.Empty;

        public static string Formatar(this double? number, string formato = FormatoMoeda) => number.HasValue ? number.Value.Formatar(formato) : string.Empty;

        public static string FormatarMoeda(this double? number) => number.HasValue ? number.Value.ToString("C", Cultura) : string.Empty;

        public static string Formatar(this DateTime? dateTime) => dateTime.HasValue ? dateTime.Value.ToString("dd/MM/yyyy") : string.Empty;

        public static string FormatarDataHora(this DateTime? dateTime) => dateTime.HasValue ? dateTime.Value.ToString("dd/MM/yyyy hh:mm:ss") : string.Empty;

        public static string Formatar(this TimeSpan? timeSpan) => timeSpan.HasValue ? timeSpan.Value.ToString() : string.Empty;
    }
}
